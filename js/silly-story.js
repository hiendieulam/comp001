...
function result() {
	...
	var keyrandom = Math.floor(Math.random() * insertX.length);
	var xItem = insertX[keyrandom];
	var yItem = insertY[keyrandom];
	var zItem = insertZ[keyrandom];
	...
	var str = newStory;
	var str = str.replace(/:insertx:/g, xItem);
	var str = str.replace(':inserty:', yItem);
	var str = str.replace(':insertz:', zItem);

	/* STEP 10: If the user has typed a name in the customName field, replace the name 'Bob' in the story with whatever they typed */
	if(customName.value != '') {
		var newcustomName = customName.value;
		var str = str.replace('Bob', newcustomName);
		console.log(str);
	}
	...
	/* STEP 11: If the metric radio button has been checked, we need to convert the temperature and mass numbers in the story */
  	if(document.getElementById("metric").checked) {
		...
		var uploadweight = 300 * 0.453592;
		// STEP 11b: Replace the string 300 pounds with the updated weight in kg
		str = str.replace('300 pounds', uploadweight + ' kgs');
		...
		str = str.replace('94 farenheit',temperature);
 	 }
...
var evenx = 'randomize';
var evenx = document.getElementById("randomize");
